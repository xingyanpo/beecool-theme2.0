...
`;
        ...
        case "sweep":
          animation = new CustomAnimation(new SequenceEffect([
            ...
            new ParallelEffect(textElements.map((item, index) => {
              return new CustomKeyframeEffect(item, { opacity: [0, 1], visibility: ["hidden", "visible"], clipPath: ["inset(0 0 100% 0)", "inset(0 0 0 0)"], transform: ["translateX(100%)", "translateX(0)"] }, { duration: 450, delay: 100 * index, easing: "cubic-bezier(0.5, 0.06, 0.01, 0.99)" });
            }))
          ]));
          break;
        case "fade":
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      ...
      switch (name) {
        case "open":
          ...
          if(widthSize<1200){
            if(this.open){
              $(".header_lastSvg").html( `<svg focusable="false" width="18" height="14" class="icon icon--header-close" viewBox="0 0 14 14"><path d="M13 13L1 1M13 1L1 13" stroke="currentColor" stroke-width="2" fill="none"></path></svg>`)
            }else{
              $(".header_lastSvg").html(`<svg focusable="false" width="18" height="14" class="icon icon--header-hamburger" viewBox="0 0 18 14"><path d="M0 1h18M0 13h18H0zm0-6h18H0z" fill="none" stroke="currentColor" stroke-width="2"></path></svg>`)
            }
          }
  ...
  let priceNew = (variant.compare_at_price - variant.price)/100;
      $(".priceNew").html("Save $"+priceNew.toFixed(2) )

    
    

      let prive_value = (Math.round((Number(variant.compare_at_price)- Number( variant.price)) / Number(variant.compare_at_price) * 10000) / 100.0)
      let save_percentage = prive_value.toFixed(0) + "%"

      $(".priceNewPercentage").html("Save "+save_percentage)
  ...
  var ProductFacet = class extends CustomHTMLElement {
    ...
    async _rerender(event) {
      ...
      try {
        ...
        $(".product-facet__filter-item .filter_value_loadMore").click(function(){
          $(this).parent().find(".checkbox-containers").show();
          $(this).hide();
  
          let cName = $(this).parents('.collapsible').prev().text().trim()
        ...
        let cateVal = document.querySelectorAll('.collapsible-toggle')
        cateVal.forEach(dom =>{
          let nodeText = dom.textContent.trim()
          console.log(getCookie(nodeText))
          if(getCookie(nodeText)){
            let tet = nodeText.toLowerCase().replaceAll(" ","_")
            $('#'+tet).hide()
            $('#'+tet).siblings().css('display','block')
          }
        })
